# Wishlist & Purchase-Blocker Discovery Engine (Myntra)

A small, four-stage pipeline that turns public customer chatter into a ranked,
countable list of **what stops fashion shoppers from buying**, **why they save to
wishlist**, and **where they go off-platform to decide**. Built to identify,
quantify, and compare opportunity areas for user-interview recruiting and
product prioritization — not to produce statistically representative metrics.

It is deliberately simple: flat files and scripts. No UI, no database, no
scheduler, no ML/clustering. The blocker categories are a fixed list on purpose —
that's what makes results countable and comparable over time.

**Pipeline:** `collect.py` (scrape) → `extract.py` (LLM structured extraction) →
`aggregate.py` (CSV cuts) → `discover.py` (discovery report answering the product
questions and ranking opportunity areas).
...
| File | Role |
| ---- | ---- |
| `collect.py` | Stage 1 — scrape & normalize raw text |
